...
const totalCategories = computed(
  () => new Set(products.value.map((p) => p.category?.id)).size,
);
const lowStockItems = computed(
  () => products.value.filter((p) => p.quantity <= p.minQuantity).length,
);
...
const confirmDelete = async () => {
  ...
  try {
    await axios.delete(`${baseUrl}/products/${productToDelete.value._id}/`, {
      headers,
    });
    products.value = products.value.filter(
      (p) => p.id !== productToDelete.value._id,
    );
    closeDeleteModal();
  } catch (error) {
...
const getCategoryIcon = (category) => {
  const icons = {
    Electronics:
      "M9 3v2m6-2v2M9 3h6M9 3H5a2 2 0 00-2 2v12a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2h-4M9 7h6",
    Clothing:
            ...
            >
                  ...
                  >
                    ...
                    >
                      <path
                        ...
                        stroke-linejoin="round"
                        stroke-width="2"
                        :d="getCategoryIcon(product.category?.name)"
                      />
                    </svg>
                  </div>
                  <div>
              ...
              <span
                :class="[
                  ...
                  getProductStatus(product) === 'Expired'
                    ? 'bg-red-100 text-red-700'
                    : getProductStatus(product) === 'In Stock'
